...
def get_args_parser():
    ...
    parser.add_argument('--train-metadata-aux', default=None, nargs='+',
    ...
    parser.add_argument('--freeze-temperature', action='store_true', dest='freeze_temperature')
    parser.add_argument('--no-freeze-temperature', action='store_false', dest='freeze_temperature')
    ...
    parser.add_argument('--pretrain-zoo', default='openai', type=str, choices=['openai', 'open_clip', 'lavila'])
...
def main(args):
    ...
    if args.train_metadata_aux is not None:
        ...
        for aux_i, aux_pkl in enumerate(args.train_metadata_aux):
            aux_dataset = VideoCaptionDatasetCLIP(
                args.dataset, args.root, aux_pkl,
                transform=train_transform, is_training=True, tokenizer=tokenizer,
                clip_length=args.clip_length, clip_stride=args.clip_stride,
                chunk_len=args.video_chunk_length,
                fast_rrc=args.fused_decode_crop, rrc_params=(crop_size, (0.5, 1.0)),
    ...
    val_dataset = VideoCaptionDatasetCLIP(
        'ek100_mir', args.root_val, args.val_metadata,
